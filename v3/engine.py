"""Independent causal linear-perpetual research engine. No order submission.
...
@njit(cache=True)
def _run(m,ts,sig,stop,start,end,p,record):
    ...
    ledger=np.empty((2*n if record else 0, 16)); nt=0
        ...
        if q==0 and not weeklock and not daylock and i>=int(delay) and ts[i]>last_exit and ts[i]-last_exit>=cool and daily_entries<int(maxday):
            ...
            if s!=0 and np.isfinite(sf) and 0<sf<.95:
                ...
                if quant>0 and budget>1e-10 and ent0>0 and quant*ent0>=min_notional and valid_target:
                    ...
                    funding=0.;eind=i;estop=dist;st=ent-s*dist;target=ent+s*dist*rr
...
def run(m,ts,signals,stops,cfg=Config(),start=0,end=None,record=False):
    ...
    return _run(m,ts,signals,stops,start,end,p,record)
