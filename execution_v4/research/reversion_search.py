...
def main():
 ...
 for family,tf,look,volume,trend,direction in itertools.product(['counter_breakout','counter_trend_state','rsi2'],[15,60,240],[12,48,192],[0.,1.],[0,200],['both','long']):
  if family=='rsi2' and look!=48:continue
  if family=='counter_trend_state' and trend!=200:continue
  for stop,rr,trail in [(1.5,1.5,0.),(2.5,1.,0.),(4.,.5,0.),(6.,.25,0.)]:
   specs.append(StrategySpec(minimum_price=0. if a.symbol=='XRPUSDT' else 1000.,family=family,timeframe_minutes=tf,lookback=look,volume_multiple=volume,trend_span=trend,direction=direction,stop_atr=stop,target_r=rr,trail_r=trail,max_hold_hours=24,risk_fraction=.03,exposure_cap=5,daily_loss=.10,weekly_loss=.25))
 if a.limit:specs=specs[:a.limit]
 protocol={'symbol':a.symbol,'instrument_grid_assumptions':{'ETHUSDT':[.001,.01,20.],'BTCUSDT':[.001,.1,100.],'XRPUSDT':[1.,.0001,5.]},'created_at_utc':pd.Timestamp.now(tz='UTC').isoformat(),'periods':PERIODS,'grid':[asdict(s) for s in specs],'data_sha256':sha256(a.data),'research_extension':'Countertrend and short-horizon mean reversion, after previous families. Same fixed dates; thresholds selected only using development/validation. Not an untouched global holdout.','selection':'Top 24 development candidates with >=100 trades,no liquidations,DD<=60%; tune risk on validation only; primary max validation CAGR with >=30 trades and DD<=50%; freeze before evaluation; all finalists reported. Prior project calendar exposure remains.','targets':{'evaluation_weeks':150,'evaluation_trades':200,'cagr':2.0,'rolling_365_day_minimum':2.0},'costs':{'fee_bps_per_side':5,'slippage_bps_per_side':2,'prior_minute_volume_participation':.01}}
