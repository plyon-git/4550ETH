"""V5 separate engine derived from the preserved V4 simulator. Adds absolute target levels. No order submission.
...
@njit(cache=True)
def _run(m,ts,sig,stop,absolute_target,start,end,p,record):
    ...
    bal=initial; q=0.; ent=0.; st=0.; target=0.; efee=0.; funding=0.; eind=0; estop=0.; pre_equity=initial
    ...
    for i in range(start,end):
        ...
        if q!=0:
            ...
            elif i>eind and ts[i]-ts[eind]>=hold:reason=4;px=op;exit_at_open=True
        ...
        if q==0 and not weeklock and not daylock and i>=int(delay) and ts[i]>last_exit and ts[i]-last_exit>=cool and daily_entries<int(maxday):
            ...
            if s!=0 and np.isfinite(sf) and 0<sf<.95:
                ent0=op*(1+s*slip)
                ...
                if quant>0 and budget>1e-10 and ent0>0 and quant*ent0>=min_notional and valid_target:
                    ...
                    funding=0.;eind=i;estop=dist;st=ent-s*dist;target=candidate_target
...
def run(m,ts,signals,stops,absolute_target,cfg=Config(),start=0,end=None,record=False):
    ...
    return _run(m,ts,signals,stops,absolute_target,start,end,p,record)
